...
namespace Opm
{
    ...
    void SatFuncStone2::evalKr(const double* s, double* kr) const
    {
	  ...
	  double sw = max(s[Aqua],Swco);
            double sg = s[Vapour];
            double so = s[Liquid];            
            double krocw = krocw_;
	    double xw = (sw-Swco)/max(sg + sw - Swco, eps);
	    double xg = 1-xw;
	    double ssw= sg +sw;
	    double sso= so;
	    double ssg = sw - Swco +sg;
	    double krw = krw_(ssw);
            double krg = krg_(ssg);
            double krow = krow_(sso); // = 1 - so
            double krog = krog_(sso);      // = 1 - so - sw
    ...
    void SatFuncStone2::evalKrDeriv(const double* s, double* kr, double* dkrds) const
    {
	   ...
	   double sw = max(s[Aqua],Swco);
	    ...
	    double ssg = sw - Swco +sg;
	    double krw = krw_(ssw);
	    double dkrw = krw_.derivative(ssw);
            double krg = krg_(ssg);
	    double dkrg = krg_.derivative(ssg);
            double krow = krow_(sso); // = 1 - so
	    double dkrow = krow_.derivative(sso);
            double krog = krog_(sso);      // = 1 - so - sw
	    double dkrog = krog_.derivative(sw + sg);
            kr[Aqua] = xw*krw;
            kr[Vapour] = xg*krg;
            kr[Liquid] = xw*krow + xg*krog; 
